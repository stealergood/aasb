<script type="text/javascript">
    ...
    function reset_input() {
        document.getElementById("izin").disabled = true;
        document.getElementById("hadir").checked = true;
        document.getElementById("hadir").disabled = false;
        document.getElementById("form_izin").disabled = false;
        <?php
    ...
    $(document).on("click", "#simpan", function() {
        ...
        if (jam_mulai == '' || jam_hingga == '') {
            $('#jam_mulai').trigger('focus')
            $("#jam_mulai_help").text('Jam Masih Kosong');
            return
        }
        ...
        formData.append('jam', jam_mulai + ' - ' + jam_hingga);
        formData.append('kelas', kelas);

        if (presensi == 'H') {
            formData.append('keterangan', 'H');
            formData.append('file_izin', '');
        } else {
            formData.append('file_izin', $('#file_izin').prop('files')[0]);
            formData.append('keterangan', jenis_izin);
            formData.append('kode_izin', kode_izin);
        }

        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('absensi/add') ?>",
            data: formData,
            dataType: "json",
            cache: false,
            processData: false,
